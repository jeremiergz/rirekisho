...
function Lang({ dense = false, item }: LangProps): JSX.Element {
  ...
  return (
    <div className={clsx('flex flex-col items-center justify-center', !dense && 'ml-3')}>
      <div className={clsx('w-10 md:w-12', 'print:w-12')}>
        <GatsbyImage alt={item.name} image={image} />
      </div>
      <div className={clsx('mt-0.5 flex md:mt-1', 'print:mt-1')}>
        {fullStarsNumber.map(fstar => (
          <Star className={clsx('h-3 md:h-4', 'print:h-4')} key={fstar} type="full" />
        ))}
        {halfStarsNumber.map(fstar => (
          <Star className={clsx('h-3 md:h-4', 'print:h-4')} key={fstar} type="half" />
        ))}
        {emptyStarsNumber.map(fstar => (
          <Star className={clsx('h-3 md:h-4', 'print:h-4')} key={fstar} type="empty" />
        ))}
      </div>
    </div>
  );
